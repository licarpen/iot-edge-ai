...
def infer_on_video(args):
    ...
    while cap.isOpened():
        ...
        ### TO CHECK: Pre-process the frame
        image = cv2.resize(frame, (input_shape[3], input_shape[2]))
        image = image.transpose((2,0,1))
        image = image.reshape(1, 3, input_shape[2], input_shape[3])

        ### TO CHECK: Perform inference on the frame
        plugin.async_inference(image)

        ### TO CHECK: Get the output of inference
        if plugin.wait() == 0:
            result = plugin.extract_output()
        ### TO CHECK: Update the frame to include detected bounding boxes
